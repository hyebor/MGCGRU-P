...
class DCGRU:
    ...
    def __init__(self, adj_mx, input_dim, num_units, max_diffusion_step, scope):
        ...
        self._num_nodes = adj_mx[0].shape[0]
        self._num_units = num_units
        self._max_diffusion_step = max_diffusion_step
        self._supportsNadj = []

        for adj_mxi in adj_mx:
            self._supports = []
            supports = []

            supports.append(calculate_random_walk_matrix(adj_mxi).T)
            supports.append(calculate_random_walk_matrix(adj_mxi.T).T)

            for support in supports:
                self._supports.append(self._build_sparse_matrix(support))
                # self._supports.append(support)
            self._supportsNadj.append(self._supports)
        # print(self._num_units)
        # print(len(self._supports))
        self.num_matrices = len(self._supports) * self._max_diffusion_step + 1
        # print(self.num_matrices)
        ...
        with tf.variable_scope(scope, reuse=False):  #
            ...
            self.Wr = tf.get_variable('Wr',
                                      [len(self._supportsNadj) * self.input_size * self.num_matrices, self._num_units],
                                      dtype=tf.float32,
                                      initializer=tf.glorot_uniform_initializer())

            self.Wu = tf.get_variable('Wu',
                                      [len(self._supportsNadj) * self.input_size * self.num_matrices, self._num_units],
                                      dtype=tf.float32,
                                      initializer=tf.glorot_uniform_initializer())
    ...
    def _gconv(self, inputs, state, output_size, weights, biases):
        ...
        num_s = 0
        for supports in self._supportsNadj:
            x = inputs_and_state
            ...
            for support in supports:
                ...
                x1 = tf.sparse_tensor_dense_matmul(support, x0)  # , a_is_sparse = True
                x = self._concat(x, x1)

                for k in range(2, self._max_diffusion_step + 1):
                    x2 = 2 * tf.sparse_tensor_dense_matmul(support, x1) - x0  # , a_is_sparse = True
                    x = self._concat(x, x2)
                    x1, x0 = x2, x1
            ...
            if num_s == 0:
                ...
                x_ini = tf.concat([x_ini, x], axis=1)
            num_s += 1

        x = tf.matmul(x_ini, weights)
...
def calculate_random_walk_matrix(adj_mx):
    ...
    d = np.array(adj_mx.sum(1)).astype('float32')
    d_inv = np.power(d, -1).flatten()
    d_inv[np.isinf(d_inv)] = 0.
    d_mat_inv = sp.diags(d_inv)
    random_walk_mx = d_mat_inv.dot(adj_mx).tocoo()
    # print(random_walk_mx.dtype)
...
def linear_projection(state, weight, weight_delta, noise=True):
    ...
    if noise:
        measurement = measurement_mean + noise_std_dev * tf.random.normal(shape=(tf.shape(measurement_mean)), mean=0,
                                                                          stddev=1)
...
def particle_flow(particles, measurement, weight, weight_delta):
    ...
    PH_t_R_inv_z = tf.tile(R_inv[:, tf.newaxis, tf.newaxis], [1, num_units, num_nodes]) * tf.einsum('ijk,ikl->ijl',
                                                                                                    PH_t, measurement)
...
def prediction_batch(particles_0, particles, init_measurement, target, rnn_0, rnn_1, weight, weight_delta,
                     node_embedding,
...
def training(inputs, targets, rnn_0, rnn_1, weight, weight_delta,
    ...
    prediction_samples, prediction_mu, prediction_sigma = prediction_batch(particles_0, particles, inputs[:, -1, :, :],
                                                                           targets, rnn_0, rnn_1, weight,
                                                                           weight_delta, node_embedding, is_training,
                                                                           curriculam_prob, rnn_type)
    return prediction_samples, prediction_mu, prediction_sigma
